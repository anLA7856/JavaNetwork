 ...
 * 一个重定向的服务器。
 * 用法：
 * 首先启动Redirector，必须传入两个参数，第一个是重定向后的网站，比如www.final.com，第二个是服务器的端口
 * 
 * 接着，在命令行中输入telnet localhost +端口
 * 就会有一个socket连入到这个类的serversocket。
 * 
 * 接着，输入你要访问的网站比如www.tencent.com
 * 然后该Redirector就会把页面重定向到www.final.com，即模拟将final的数据显示给你。
 * @author anla7856
 *
 */
